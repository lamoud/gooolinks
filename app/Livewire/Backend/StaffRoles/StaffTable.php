...
class StaffTable extends DataTableComponent
{
    ...
    {
        return User::permission('admin_view');
        //->with('roles')
    ...
    public function columns(): array
    {
        ...
        return [
            ...
            Column::make(__('Role'))
                ->label(function ($row) {
                    if ($row->getRoleNames()) {
                        $displayNames = array_column($row->roles->toArray(), 'display_name');
            ...
            Column::make(__('Actions'))
                ->label(function ($row) {
                    return view('livewire.backend.staff-roles.actions.staff-actions', ['row' => $row]); // قم بإنشاء view خاص بالإجراءات
                })
        ];
    ...
    {
        return $this->dispatch('deleteUser', id: $id, type: 'question', msg: 'هل ترغب في حذف الموظف؟');
    }
